import { HeartIcon, 
    VolumeUpIcon as VolumeDownIcom } from '@heroicons/react/outline';
...
const Player = () => {
    ...
    return (
        <div className="h-24 bg-gradient-to-b from-black to-gray-900 text-white grid grid-cols-3 text-xs md:text-base px-2 md:px-8">
            <div className="flex items-center spce-x-4">
                <img 
                    className="hidden md:inline h-10 w-10" src={songInfo?.album.images?.[0]?.url} 
                    alt="" 
                />
                <div>
                    <h3>{songInfo?.name}</h3>
            ...
            <div className="flex items-center space-x-3 md:space-x-4 justify-end pr-5">
                <VolumeDownIcom onClick={() => volume > 0 && setVolume(volume - 10)} className="button" />
